...
| Cohen's d | Small (d < 0.2) |
| Decision | **REJECT H₀** |

> **Figure 1** — Distribution of HFVS Dimension Scores and Composite (2×3 histogram panel with KDE overlays and Normal fit). *Source: `fig1_histograms_hfvs.png` — Cell B.1.*

**Interpretation:** The national mean HFVS is 0.480, significantly below the neutral midpoint of 0.5 (p ≪ 0.001), indicating that the **aggregate** population sits in the moderate-low vulnerability range. The effect size is small (d < 0.2) — the gap is statistically detectable at n=21,347 but not substantively large. The right-skewed housing burden ratio (skew = +2.58) reveals a severe vulnerable tail — aggregate affordability conceals substantial individual hardship.

> **Policy note:** The IRA should not use the national mean as a pricing reference. The relevant statistic is the **top-quartile threshold**, which identifies the ~5,337 highest-vulnerability households requiring priority intervention.
...
> **Figure 2a** — HFVS Composite by Stratum (box plot: Urban vs Rural). *Source: `fig2_boxplots_group.png` — Cell B.2, left panel.*
...
| Statistic | Value |
|---|---|
| Levene's F | Heterogeneous variances noted (Welch ANOVA preferred; Tukey HSD robust at large n) |
| F(2, 21344) | **197.6** |
| p-value | < 0.001 *** |
| η² | **0.018 (small)** |
| Decision | **REJECT H₀** |
...
**Education is a monotone protective factor** (more education → lower HFVS), but η²=0.018 means education explains only **1.8% of HFVS variance** — it improves financial stress (D1) through income, but does not protect against hazard exposure (D3) or dwelling quality (D4), which depend on location and housing stock.

> **Figure 2b** — HFVS Composite by Education Tier (box plot: No Education / Primary-Secondary / Post-secondary). *Source: `fig2_boxplots_group.png` — Cell B.2, right panel.*

> **Figure V.2** — Education Gradient and Effect Size Context (box plots with Tukey HSD significance brackets + η² bar). *Source: `fig_V2_education_boxplot_effectsize.png` — Cell V.2.*
...
> **Note:** The Beta parameters below are computed dynamically from the notebook run. The implied mean must closely match the observed composite mean (0.480). The parameters listed in earlier draft versions (α=28.67, β=44.97, implied mean=0.389) derive from a pre-correction run and should not be used.

| MLE Statistic | Value |
|---|---|
| Log-likelihood | (computed at runtime) |
| AIC | (computed at runtime) |
| BIC | (computed at runtime) |
| Implied mean (Beta) | Must match observed mean ≈ **0.480** |
| Observed mean | **0.480** |
...
> **Figure 9** — MLE Beta(α, β) Fit to HFVS Composite (histogram with Beta PDF overlay). *Source: `fig9_mle_beta_fit.png` — Cell C.8.*
...
| Statistic | Value |
|---|---|
| Spearman ρ (D1 ↔ Composite) | **0.624** |
| p-value | < 10⁻¹⁹² *** |
| 95% CI for ρ (Fisher's Z) | [0.614, 0.634] |
| Decision | **REJECT H₀** |
...
15. World Bank Open Data. (2024). Kenya housing and urban development indicators. https://data.worldbank.org **Cross-validation of findings:** Where both methods agree, the conclusion is robust. Any disagreement triggers investigation.
